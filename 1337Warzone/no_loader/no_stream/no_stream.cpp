...
BLOWFISH::BLOWFISH(std::string hexKey)
{
	...
	databyte* key = new databyte[hexKey.length() / 2];//
...
BLOWFISH::BLOWFISH(databyte* cipherKey, int keyLength)
{
	IvSet = false;
	SetupKey(cipherKey, keyLength);
}

databyte* BLOWFISH::Encrypt_ECB(databyte* data, int length, int* newlength)
{
	return Crypt_ECB(data, length, newlength, &BLOWFISH::Encrypt_Block, false);
}

databyte* BLOWFISH::Decrypt_ECB(databyte* data, int length, int* newlength)
{
	return Crypt_ECB(data, length, newlength, &BLOWFISH::Decrypt_Block, true);
}

databyte* BLOWFISH::Encrypt_CBC(databyte* data, int length, int* newlength)
{
	return Crypt_CBC(data, length, newlength, &BLOWFISH::Encrypt_Block, false);
}

databyte* BLOWFISH::Decrypt_CBC(databyte* data, int length, int* newlength)
{
...
{
	databyte* binaryData = new databyte[data.length()];
	for (int i = 0; i < data.length(); i++)
		binaryData[i] = data[i];
	int newlen = 0;
	databyte* result = Encrypt_CBC(binaryData, data.length(), &newlen);
	std::string encoded = "";
...
std::string BLOWFISH::Decrypt_CBC(std::string data)
{
	...
	databyte* binaryData = new databyte[data.length() / 2];//
	...
	int len = 0;
	databyte* cryptresult = Decrypt_CBC(binaryData, data.length() / 2, &len);
	std::string result = "";
	for (int i = 0; i < len; i++)
...
databyte* BLOWFISH::padData(databyte* data, int length, int* paddedLength, bool decrypt, bool IvSpace = false)
{
	int offset = 0;
	int dataoffset = 0;
	if (decrypt)
	...
	databyte* pData = new databyte[*paddedLength];
...
int BLOWFISH::findPaddingEnd(databyte* data, int length)
{
	int i = length;
...
databyte* BLOWFISH::Crypt_ECB(databyte* data, int length, int* newlength, void (BLOWFISH::* CryptBlock)(databyte*, int), bool decrypt)
{
	databyte* pData;
...
databyte* BLOWFISH::Crypt_CBC(databyte* data, int length, int* newlength, void (BLOWFISH::* CryptBlock)(databyte*, int), bool decrypt)
{
	databyte* pData;
	if (!decrypt && !IvSet)
		SetRandomIV();
	...
	if (decrypt) //if we are decrypting, we have to find where the data ends, and remove the IV
	{
		*newlength = findPaddingEnd(pData, *newlength) - 8;
		databyte* noIV = new databyte[*newlength];
...
void BLOWFISH::SetIV(databyte* newIV)
{
	IvSet = true;
	for (int i = 0; i < 8; i++)
		IV[i] = newIV[i];
...
databyte* BLOWFISH::GetIV()
{
	databyte* returnIV = new databyte[8];
	for (int i = 0; i < 8; i++)
		returnIV[i] = IV[i];
	return returnIV;
}

void BLOWFISH::Encrypt_Block(databyte* block, int offset)
{
...
void BLOWFISH::Decrypt_Block(databyte* block, int offset)
{
	setblock(block, offset);
	decipher();
	getblock(block, offset);
...
void BLOWFISH::setblock(databyte* block, int offset)
{
...
void BLOWFISH::getblock(databyte* block, int offset)
{
	//TODO: CHECK ENDIANNESS
...
void BLOWFISH::SetupKey(databyte* cipherKey, int length)
{
	if (length > 56)
	{
		throw 56;
	...
	databyte* key = new databyte[length]; //
